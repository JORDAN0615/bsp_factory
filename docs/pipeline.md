...
| `deep_planner` (Deep Agents, optional · ADR-0020) | `issue`, retry context, repo, skills and `search_bsp_knowledge` tools | Reads and investigates on demand, then returns a repair guide for the executor |
| `deep_patch_agent` (Deep Agents · ADR-0016/0017) | `issue`, optional guide, retry context, staging worktree, skills and `search_bsp_knowledge` tools | Replaces `inspect_repo` + `patch_agent` when `DEEP_AGENT_ENABLED=true`; retrieves knowledge on demand, plans, explores, delegates read-only research, edits staging, and returns its real git diff or `NO_PATCH` |
...
|---|---|---|
| `Deep Agent / planner enabled?` | `route_after_classify_error` | legacy → `select_skills`; deep-only → `deep_patch_agent`; deep+planner → `deep_planner` |
