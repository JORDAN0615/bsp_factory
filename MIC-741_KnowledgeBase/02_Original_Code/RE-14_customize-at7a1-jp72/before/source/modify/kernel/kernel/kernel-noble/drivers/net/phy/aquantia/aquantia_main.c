...
/* ADV: Add LED fix */
static int aqr113c_read_status_with_led_fix(struct phy_device *phydev)
{
	int ret, val;
	struct aqr107_priv *priv = phydev->priv;
	...
	val = phy_read_mmd(phydev, MDIO_MMD_VEND1, 0xc430);

	if (priv->led_mode0 != -1 && val != priv->led_mode0) {
...
},
{
	...
	.config_intr    = aqr_config_intr,
	.handle_interrupt       = aqr_handle_interrupt,
	.read_status    = aqr113c_read_status_with_led_fix,
	.get_tunable    = aqr107_get_tunable,
	.set_tunable    = aqr107_set_tunable,
	.suspend        = aqr107_suspend,
	.resume         = aqr107_resume,
