			      ...
			      const u8 *data, size_t len)
{
	...
	phydev_err(phydev, "DEBUG: Start writing FW to RAM. Addr=0x%08x, Len=%ld bytes\n", addr, len);
	...
	phydev_err(phydev, "DEBUG: CRC Check - Calc: 0x%04x, PHY: 0x%04x\n", crc, up_crc);
...
int aqr_firmware_load(struct phy_device *phydev)
{
	 ...
	 */
	phydev_err(phydev, "DEBUG: >>> Enter aqr_firmware_load <<<\n");
	ret = aqr_wait_reset_complete(phydev);
	phydev_err(phydev, "DEBUG: aqr_wait_reset_complete returned: %d\n", ret);
	switch (ret) {
	case 0:
		/* Some firmware is loaded => do nothing */
		phydev_err(phydev, "DEBUG: Firmware check PASSED (ret=0). PHY is active.\n");
		return 0;
	case -ETIMEDOUT:
		 ...
		 */
		phydev_err(phydev, "DEBUG: Firmware check TIMEOUT (-110)! PHY not responding.\n");
		phydev_err(phydev, "DEBUG: Attempting fallback: load from NVMEM...\n");
		ret = aqr_firmware_load_nvmem(phydev);
		if (!ret){
			phydev_err(phydev, "DEBUG: NVMEM load SUCCESS.\n");
			return ret;
		}
		phydev_err(phydev, "DEBUG: NVMEM load failed, trying Filesystem...\n");
		ret = aqr_firmware_load_fs(phydev);
		if (ret){
			phydev_err(phydev, "DEBUG: Filesystem load FAILED with ret=%d\n", ret);
			return ret;
		}
		phydev_err(phydev, "DEBUG: Firmware loaded. Waiting 2s for PHY boot...\n");
		msleep(2000);
		break;
	default:
		/* PHY read error, propagate it to the caller */
		phydev_err(phydev, "DEBUG: Unexpected error from wait_reset: %d\n", ret);
		return ret;
	}
