...
def build_index(*, force: bool = False, kb_dir: Path | None = None) -> list[Chunk]:
    """Load or build index.
    ...
    Configure KB location via kb_dir param or KB_DIR env var.
    Default: RAG_DOCS/MIC-741_KnowledgeBase (matches mic741_knowledge_source_dir).

    Args:
        force: if True, always rebuild regardless of cache state.
        kb_dir: explicit KB directory; overrides KB_DIR env var.
    """
    if kb_dir is None:
        kb_dir = Path(__file__).parent.parent / os.getenv("KB_DIR", "RAG_DOCS/MIC-741_KnowledgeBase")
